  ...
  <div class="main-container">
    ...
    <!-- edit dialog -->
      <md-dialog :md-active.sync="editDialog">
        <md-dialog-title>Update Team Data</md-dialog-title>
        <md-dialog-content>
          <EditTeam
    ...
    <!-- add dialog -->
      <md-dialog :md-active.sync="addDialog">
        <md-dialog-title>Add New Member</md-dialog-title>
        <md-dialog-content>
          <AddTeam
          v-on:close-dialog="refresh"/>
        </md-dialog-content>
    ...
    <md-table v-model="searched" md-sort="title" md-sort-order="asc" md-card md-fixed-header>
      ...
      <md-table-empty-state
        ...
        <md-button class="md-primary md-raised" @click="addData">Create New Member</md-button>
      </md-table-empty-state>

      <md-table-row slot="md-table-row" slot-scope="{ item }">
        <md-table-cell md-label="ID" md-sort-by="id" md-numeric>{{ item.id }}</md-table-cell>
        <md-table-cell md-label="Image">
          <img :src="config.imagePath + item.image" class="image-thumbnail" width="100">
        </md-table-cell>
...
export default {
  ...
  data: () => ({
    ...
    methods: {
        getData(){
            Axios.get(this.config.baseUrl + 'admin/team').then(res=>{
        ...
        onConfirm(){
          Axios.delete(this.config.baseUrl + 'admin/team/'+this.selectedId).then(res=>{
