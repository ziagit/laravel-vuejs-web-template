  ...
  <div class="main-container">
    ...
    <!-- edit dialog -->
      <md-dialog :md-active.sync="editDialog">
        <md-dialog-title>Update Technology</md-dialog-title>
        <md-dialog-content>
          <EditTech 
      ...
      <md-dialog :md-active.sync="addDialog">
        <md-dialog-title>Add a new technology</md-dialog-title>
        <md-dialog-content>
          <AddTech 
          v-on:close-dialog="refresh"/>
        </md-dialog-content>
    ...
    <md-table v-model="searched" md-sort="name" md-sort-order="asc" md-card md-fixed-header>
      <md-table-toolbar>
        ...
        <div class="md-toolbar-section-start">
          <h1 class="md-title">Technology</h1>
        </div>
      </md-table-toolbar>
      ...
      <md-table-row slot="md-table-row" slot-scope="{ item }">
        ...
        <md-table-cell md-label="Icon">
          <img :src="config.imagePath + item.logo" class="image-thumbnail" width="50">
        </md-table-cell>
        <md-table-cell md-label="Name" md-sort-by="name">{{ item.name }}</md-table-cell>
        <md-table-cell md-label="Color Brand" md-sort-by="brand">{{ item.brand }}</md-table-cell>
...
export default {
  ...
  data: () => ({
    ...
    methods: {
        getData(){
            Axios.get(this.config.baseUrl+'admin/techs').then(res=>{
        ...
        onConfirm(){
          Axios.delete(this.config.baseUrl+'admin/techs/'+this.selectedId).then(res=>{
